      ...
      {!loading && feat && (
        <button onClick={() => setActive(feat)} className="card-hover"
          ...
          <div style={{ minHeight: 300, background: feat.image ? `url(${feat.image}) center/cover` : GRADS[0], position: 'relative', display: 'flex', alignItems: 'flex-end', padding: 22 }}>
            ...
            <span className="font-cond" style={{ position: 'relative', background: 'var(--accent)', color: '#fff', fontWeight: 700, fontSize: 12, letterSpacing: '1px', padding: '4px 12px', borderRadius: 5, textTransform: 'uppercase' }}>{filter.label}</span>
          </div>
          <div style={{ background: 'var(--panel)', padding: '28px 26px', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: 11.5, color: 'var(--mut)', marginBottom: 12 }}><span style={{ color: 'var(--gold)', fontWeight: 700 }}>LiveScoreFast</span><span>·</span><span>{timeAgo(feat.date)}</span></div>
          ...
          {rest.map((a, i) => (
            <button key={i} onClick={() => setActive(a)} className="card-hover"
              ...
              <div style={{ height: 150, background: a.image ? `url(${a.image}) center/cover` : GRADS[(i + 1) % GRADS.length], position: 'relative', display: 'flex', alignItems: 'flex-start', padding: 12 }}>
                <span className="font-cond" style={{ background: 'rgba(0,0,0,.45)', color: '#fff', fontWeight: 700, fontSize: 11, letterSpacing: '.5px', padding: '3px 9px', borderRadius: 4, textTransform: 'uppercase' }}>{filter.label}</span>
              </div>
              <div style={{ padding: '15px 16px 17px', display: 'flex', flexDirection: 'column', flex: 1 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 11, color: 'var(--mut)' }}><span style={{ color: 'var(--gold)', fontWeight: 700 }}>LiveScoreFast</span><span>·</span><span>{timeAgo(a.date)}</span></div>
              </div>
            </button>
          ))}
        </div>
          ...
          <div onClick={e => e.stopPropagation()} className="panel" style={{ maxWidth: 680, width: '100%', borderRadius: 12 }}>
            ...
            <div style={{ padding: '22px 26px 26px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: 12, color: 'var(--mut)', marginBottom: 12 }}>
                <span className="font-cond" style={{ background: 'var(--accent)', color: '#fff', fontWeight: 700, fontSize: 11, letterSpacing: '.5px', padding: '3px 10px', borderRadius: 4, textTransform: 'uppercase' }}>{filter.label}</span>
                <span style={{ color: 'var(--gold)', fontWeight: 700 }}>LiveScoreFast</span>
              ...
              <p style={{ fontSize: 11.5, color: '#55617a', marginTop: 18 }}>Summary by LiveScoreFast.</p>
            </div>
          </div>
        </div>
      )}
