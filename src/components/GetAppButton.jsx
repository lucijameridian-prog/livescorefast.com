      ...
      {open && (
        <div onClick={() => setOpen(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.7)', zIndex: 200, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: '40px 16px', overflowY: 'auto' }}>
          <div onClick={e => e.stopPropagation()} className="panel" style={{ maxWidth: 640, width: '100%', borderRadius: 14 }}>
            ...
            {standalone && (
              <div style={{ margin: '16px 22px 0', padding: '12px 14px', borderRadius: 10, background: 'rgba(31,168,74,.12)', border: '1px solid rgba(31,168,74,.35)', color: 'var(--green)', fontSize: 13, fontWeight: 600 }}>✓ You're already using the installed app. Enjoy!</div>
            )}
            {!standalone && plat !== 'other' && (
              <div style={{ margin: '16px 22px 0', padding: '12px 14px', borderRadius: 10, background: 'rgba(246,201,21,.10)', border: '1px solid rgba(246,201,21,.3)', color: '#e9d9a0', fontSize: 13, lineHeight: 1.5 }}>
                📱 We detected your device is <b style={{ color: 'var(--gold)' }}>{plat === 'ios' ? 'iPhone / iPad' : 'Android'}</b> — follow the highlighted steps to add LiveScoreFast to your home screen.
              </div>
            )}

            <div style={{ padding: 22, display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(250px,1fr))', gap: 16 }}>
              {/* iOS */}
              <div style={{ border: `1px solid ${plat === 'ios' ? 'var(--gold)' : 'var(--line)'}`, borderRadius: 12, padding: 18, background: plat === 'ios' ? 'rgba(246,201,21,.06)' : 'rgba(255,255,255,.02)', order: plat === 'ios' ? -1 : 0 }}>
              ...
              {/* Android */}
              <div style={{ border: `1px solid ${plat === 'android' ? 'var(--gold)' : 'var(--line)'}`, borderRadius: 12, padding: 18, background: plat === 'android' ? 'rgba(246,201,21,.06)' : 'rgba(255,255,255,.02)', order: plat === 'android' ? -1 : 0 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
